                ...
                <div class="absolute inset-0 z-20 flex flex-col items-center justify-center text-center px-4">
                    ...
                    @if($banner->link)
                        <div class="animate-fade-in-up delay-200">
                            <a href="{{ route('client.products.index') }}" class="group relative inline-flex items-center gap-3 px-8 py-4 bg-white text-slate-900 font-bold text-sm tracking-widest uppercase rounded-full overflow-hidden transition-all hover:bg-indigo-600 hover:text-white hover:shadow-lg hover:shadow-indigo-500/50">
...
<div class="container mx-auto px-4 py-16 space-y-24">
    ...
    <section class="relative bg-slate-900 rounded-3xl p-10 md:p-20 text-center overflow-hidden">
        ...
        <div class="relative z-10 max-w-2xl mx-auto">
            ...
            <p class="text-slate-300 mb-8 text-lg">Đăng ký nhận tin để nhận mã giảm giá <span class="text-indigo-400 font-bold">10%</span> cho đơn hàng đầu tiên.</p>
            
            <form action="{{ route('subscribe') }}" method="POST" class="flex flex-col sm:flex-row gap-3 items-start">
                @csrf
                <div class="w-full flex-1 text-left">
                    <input type="email" name="email" value="{{ old('email') }}" placeholder="Nhập email của bạn..." class="w-full px-6 py-4 rounded-full bg-white/10 border border-white/20 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all @error('email') border-rose-500 ring-1 ring-rose-500 @enderror">
                    @error('email')
                        <p class="text-rose-400 text-sm mt-2 ml-4"><i class="fa-solid fa-circle-exclamation mr-1"></i> {{ $message }}</p>
                    @enderror
                </div>
                <button type="submit" class="w-full sm:w-auto px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-full transition-all shadow-lg hover:shadow-indigo-500/50 whitespace-nowrap">Đăng ký ngay</button>
            </form>
