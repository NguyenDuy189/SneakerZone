        ...
        <div class="container mx-auto px-4 md:px-6">
            <div class="flex items-center justify-between">
                ...
                <nav class="hidden lg:flex items-center gap-8">
                    ...
                    <a href="{{ route('client.products.men') }} "class="text-sm font-bold text-slate-600 hover:text-indigo-600 uppercase tracking-wide transition-colors">NAM</a>
                    <a href="{{ route('client.products.women') }}" class="text-sm font-bold text-slate-600 hover:text-indigo-600 uppercase tracking-wide transition-colors">NỮ</a>
                    <a href="{{ route('client.products.sale') }}"
                    class="text-sm font-bold text-rose-500 hover:text-rose-600 uppercase tracking-wide transition-colors relative group">SALE
                    <span class="absolute -top-3 -right-3 text-[9px] bg-rose-600 text-white px-1 rounded animate-bounce">Hot</span></a>
                </nav>
   ...
   class="block px-4 py-2 text-sm text-slate-600 hover:bg-indigo-50 hover:text-indigo-600">
    Đăng ký
                    ...
                    {{-- Cart Icon --}}
                        ...
                        class="relative w-10 h-10 rounded-full flex items-center justify-center text-slate-500 hover:bg-slate-100 hover:text-indigo-600 transition-all">
    
                        <i class="fa fa-shopping-cart text-xl"></i>

                        {{-- Badge số lượng - Chỉ hiện khi > 0 --}}
                        <template x-if="true"> {{-- Bạn có thể dùng x-if nếu dùng Alpine, hoặc dùng PHP trực tiếp bên dưới --}}
                        @if($cartQuantity > 0)
                        <span class="absolute top-0 right-0 w-4 h-4 bg-rose-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center border border-white">
                        {{ $cartQuantity }}
                        </span>
                        @endif
                        </template>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
                ...
                <div>
                    ...
                    <ul class="space-y-3 text-sm text-slate-400">
                        <li><a href="{{ route('client.about') }}" class="hover:text-white transition-colors">Về chúng tôi</a></li>
                        <li><a href="{{ route('client.contact') }}" class="hover:text-white transition-colors">Liên hệ</a></li>
                        <li><a href="{{ route('client.stores') }}" class="hover:text-white transition-colors">Tìm cửa hàng</a></li>
                        <li><a href="{{ route('client.blog') }}" class="hover:text-white transition-colors">Tin tức</a></li>
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-3 text-sm text-slate-400">
                        <li><a href="{{ route('client.buying-guide') }}" class="hover:text-white transition-colors">Hướng dẫn mua hàng</a></li>
                        <li><a href="{{ route('client.return-policy') }}" class="hover:text-white transition-colors">Chính sách đổi trả</a></li>
                        <li><a href="{{ route('client.privacy-policy') }}" class="hover:text-white transition-colors">Chính sách bảo mật</a></li>
                        {{-- Tra cứu đơn hàng: Link tới trang danh sách đơn hàng (yêu cầu đăng nhập) --}}
                        <li><a href="{{ route('client.orders.index') }}" class="hover:text-white transition-colors">Tra cứu đơn hàng</a></li>
                    </ul>
                </div>
