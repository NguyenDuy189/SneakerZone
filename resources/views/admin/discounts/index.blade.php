...
<div class="container px-6 mx-auto mb-10 fade-in">
    ...
    {{-- 1. THỐNG KÊ NHANH (Giữ nguyên) --}}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {{-- ... (Code thống kê giữ nguyên như cũ) ... --}}
    ...
    {{-- 4. BẢNG DỮ LIỆU (ĐÃ CHỈNH SỬA) --}}
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <div class="w-full overflow-x-auto">
            {{-- Thay đổi: Thêm table-fixed và min-w-full --}}
            <table class="w-full min-w-[900px] table-fixed text-left">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100 text-xs font-bold text-slate-500 uppercase tracking-wider">
                        <th class="px-4 py-4 w-12">#</th>
                        <th class="px-4 py-4 w-[20%]">Mã Code</th>
                        <th class="px-4 py-4 w-[12%]">Giá trị</th>
                        <th class="px-4 py-4 text-center w-[10%]">Đã dùng</th>
                        {{-- Ẩn bớt trên mobile, hiện trên PC --}}
                        <th class="hidden lg:table-cell px-4 py-4 text-right w-[12%]">Min Order</th>
                        <th class="hidden xl:table-cell px-4 py-4 text-right w-[12%]">Max Giảm</th>
                        <th class="px-4 py-4 w-[15%]">Thời gian</th>
                        <th class="px-4 py-4 text-center w-[10%]">Trạng thái</th>
                        <th class="px-4 py-4 text-center w-[100px]">Hành động</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors text-sm text-slate-700">
                        ...
                        {{-- 1. Index --}}
                        <td class="px-4 py-4 text-slate-500">{{ $discounts->firstItem() + $idx }}</td>

                        {{-- 2. Mã Code --}}
                        <td class="px-4 py-4">
                            <div class="flex items-center gap-2">
                                <div class="w-8 h-8 rounded-lg bg-indigo-50 border border-indigo-100 flex-shrink-0 flex items-center justify-center text-indigo-600">
                                    <i class="fa-solid fa-ticket"></i>
                                </div>
                                <span class="font-mono font-bold text-sm text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100 border-dashed truncate max-w-full" title="{{ $item->code }}">
                                    {{ $item->code }}
                                </span>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-4">
                            @if($item->type === 'percentage')
                                ...
                                <span class="text-[10px] text-blue-600 font-medium">Theo %</span>
                            @else
                                <div class="font-bold text-slate-800">{{ number_format($item->value, 0, ',', '.') }}đ</div>
                                <span class="text-[10px] text-emerald-600 font-medium">Tiền mặt</span>
                            @endif
                        </td>
                        ...
                        <td class="px-4 py-4 text-center">
                            ...
                            <span class="text-[10px] text-slate-400 block">/ {{ $item->max_usage > 0 ? $item->max_usage : '∞' }}</span>
                        </td>

                        {{-- 5. Đơn tối thiểu (Ẩn mobile) --}}
                        <td class="hidden lg:table-cell px-4 py-4 text-right font-medium text-slate-600">
                            {{ number_format($item->min_order_amount, 0, ',', '.') }}đ
                        </td>

                        {{-- 6. Giảm tối đa (Ẩn mobile) --}}
                        <td class="hidden xl:table-cell px-4 py-4 text-right">
                            @if($item->type === 'percentage')
                                @if($item->max_discount_value > 0)
                                    <span class="font-bold text-slate-700">{{ number_format($item->max_discount_value, 0, ',', '.') }}đ</span>
                                @else
                                    <span class="text-xs text-slate-400 italic">KGH</span>
                                @endif
                            @else
                        ...
                        {{-- 7. Thời gian (Cho phép xuống dòng) --}}
                        <td class="px-4 py-4 whitespace-normal">
                            <div class="text-xs space-y-1">
                                <div class="flex flex-col xl:flex-row xl:gap-1">
                                    <span class="text-slate-400">Bắt đầu:</span> 
                                    <span class="font-medium text-slate-700">{{ $item->start_date ? $item->start_date->format('d/m/Y') : 'Ngay' }}</span>
                                </div>
                                <div class="flex flex-col xl:flex-row xl:gap-1">
                                    <span class="text-slate-400">Kết thúc:</span>
                        ...
                        <td class="px-4 py-4 text-center">
                            ...
                            @if($isExpired)
                                <span class="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-rose-100 text-rose-700 border border-rose-200">Hết hạn</span>
                            @elseif($isNotStarted)
                                <span class="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-amber-100 text-amber-700 border border-amber-200">Chờ chạy</span>
                            @elseif($isFull)
                                <span class="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-600 border border-slate-200">Hết lượt</span>
                            @else
                                <span class="inline-block px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-700 border border-emerald-200">Hoạt động</span>
                            @endif
                        </td>
