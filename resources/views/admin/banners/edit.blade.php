...
<div class="container px-6 mx-auto pb-20 max-w-5xl">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 pt-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Chỉnh sửa Banner</h1>
            <p class="text-slate-500 text-sm mt-1">Cập nhật thông tin cho banner: <span class="font-bold text-indigo-600">{{ $banner->title }}</span></p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
            ...
            <div class="md:col-span-4 space-y-6">
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wider mb-4">Hình ảnh Banner</h3>
                    ...
                    <div class="relative w-full aspect-video bg-slate-50 border-2 border-dashed border-slate-300 rounded-xl overflow-hidden hover:border-indigo-400 transition-colors group">
                        <img id="preview-img" src="{{ asset('storage/' . $banner->image_url) }}" class="w-full h-full object-cover">
                        ...
                        <label for="image-upload" class="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/0 group-hover:bg-slate-900/40 transition-all cursor-pointer">
                            ...
                            <span class="text-white text-xs font-bold mt-2 opacity-0 group-hover:opacity-100 transition-all duration-300 delay-75">Thay đổi ảnh</span>
                        </label>
                        <input type="file" name="image" id="image-upload" class="hidden" accept="image/*" onchange="previewImage(this)">
                    </div>
                    <p class="text-xs text-slate-400 mt-3 text-center">Để trống nếu không muốn thay đổi ảnh.</p>
                </div>
                ...
                <div class="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title', $banner->title) }}" placeholder="Nhập tiêu đề banner..." 
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {{-- Loop qua các option để code gọn hơn --}}
                                @php
                                    $positions = [
                                        'home_slider' => ['icon' => 'fa-images', 'color' => 'blue', 'label' => 'Home Slider', 'desc' => 'Slide chính trang chủ'],
                                        'home_mid' => ['icon' => 'fa-rectangle-ad', 'color' => 'orange', 'label' => 'Home Middle', 'desc' => 'Quảng cáo giữa trang'],
                                        'sidebar' => ['icon' => 'fa-table-columns', 'color' => 'purple', 'label' => 'Sidebar', 'desc' => 'Cột bên trang sản phẩm'],
                                        'footer' => ['icon' => 'fa-shoe-prints', 'color' => 'slate', 'label' => 'Footer', 'desc' => 'Cuối trang web'],
                                    ];
                                @endphp
                                ...
                                @foreach($positions as $key => $pos)
                                    <label class="relative flex items-center p-4 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50 hover:border-indigo-300 transition-all has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50 has-[:checked]:ring-1 has-[:checked]:ring-indigo-500">
                                        <input type="radio" name="position" value="{{ $key }}" class="sr-only" {{ old('position', $banner->position) == $key ? 'checked' : '' }}>
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-lg bg-{{ $pos['color'] }}-100 text-{{ $pos['color'] }}-600 flex items-center justify-center">
                                                <i class="fa-solid {{ $pos['icon'] }}"></i>
                                            </div>
                                            <div>
                                                <span class="block text-sm font-bold text-slate-800">{{ $pos['label'] }}</span>
                                                <span class="block text-xs text-slate-500">{{ $pos['desc'] }}</span>
                                            </div>
                                        </div>
                                    </label>
                                @endforeach
                        ...
                        <div>
                            ...
                            <textarea name="content" rows="3" placeholder="Nhập ghi chú hoặc mô tả ngắn..."
                                      class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all placeholder:text-slate-400 resize-none">{{ old('content', $banner->content) }}</textarea>
                        </div>
                    </div>
