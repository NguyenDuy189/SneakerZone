...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            // --- [QUAN TRỌNG] ĐÃ SỬA LẠI LOGIC LỌC SALE ---
            if ($request->has('is_sale')) {
            $query->where(function($subQuery) {
            $subQuery->whereHas('variants', function($q) {
            // Sửa 'price_sale' thành 'sale_price'
            // Sửa 'price' thành 'original_price' (dựa theo ảnh Database bạn gửi)
            ...
            switch ($sort) {
                case 'price_asc':  $query->orderBy('price_min', 'asc'); break;
                case 'price_desc': $query->orderBy('price_min', 'desc'); break;
            ...
            // --- BẬT CHẾ ĐỘ HIỆN LỖI ---
            // Nếu vẫn lỗi, hãy bỏ comment dòng dưới đây để xem lỗi chính xác trên màn hình:
            //dd($e->getMessage()); 

            return redirect()->route('client.home')->with('error', 'Đã xảy ra lỗi khi tải sản phẩm!');
        }
    }
    ...
    /**
 * Lọc sản phẩm theo giới tính (Nam/Nữ)
 */
public function getProductsByGender(Request $request)
{
    // Xác định giới tính dựa trên URL (men/women)
    $routeName = $request->route()->getName();
    $genderSlug = ($routeName == 'client.products.men') ? 'nam' : 'nu';

    // Gắn thêm tham số category vào request để hàm index() tự xử lý lọc
    $request->merge(['category' => $genderSlug]);

    return $this->index($request);
}

/**
 * Lấy danh sách sản phẩm đang có chương trình Sale
 */
public function getSaleProducts(Request $request)
{
    // Giả sử logic sale của bạn dựa trên việc có liên kết với FlashSale 
    // Hoặc có giá biến thể thấp hơn giá gốc. 
    // Ở đây tôi sẽ lọc các sản phẩm có is_featured hoặc bạn có thể thay bằng logic discount riêng.
    
    $request->merge(['is_sale' => true]); 
